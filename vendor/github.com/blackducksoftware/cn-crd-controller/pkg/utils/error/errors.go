...
type DeployErrors interface {
	error
	Errors() map[utils.ComponentType][]error
}

type deployErrors map[utils.ComponentType][]error

// NewDeployErrors converts a map of ComponentType:[]error to
// a DeployErrors interface
func NewDeployErrors(errMap map[utils.ComponentType][]error) DeployErrors {
	if len(errMap) == 0 {
		return nil
	}

	errs := make(map[utils.ComponentType][]error)
...
func (d deployErrors) Errors() map[utils.ComponentType][]error {
	return d
}

func ComponentErrorCount(errs error, component utils.ComponentType) int {
	if errs == nil {
		return 0
	}
