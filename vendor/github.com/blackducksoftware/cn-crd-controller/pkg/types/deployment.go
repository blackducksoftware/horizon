...
func (d *Deployment) AddMatchLabelsSelectors(new map[string]string) {
	...
	d.obj.Selector.Labels = utils.MapMerge(d.obj.Selector.Labels, new)
}
...
func (d *Deployment) RemoveMatchLabelsSelectors(remove []string) {
	for _, k := range remove {
		d.obj.Selector.Labels = utils.RemoveElement(d.obj.Selector.Labels, k)
	}
}
